...
int main(int argc, char *argv[])
{
  ...
  std::system(("rm "+output_file_path +"encoded.bin").c_str());     //removing previous encoded file
...
//   std::cout << "(" << rank << ") com buffer size = " << com_buffer_size << std::endl;
  ...
  if (rank > 0)
  {
    ...
    Encoder encoder(com_buffer_size, READ_QUEUE_SIZE, &writer_queue);
    EncoderWriter writer(&writer_queue, rank, output_file_path);
