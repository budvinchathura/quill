...
int main(int argc, char *argv[])
{
  ...
  std::string output_base_path = argv[4];
  ...
  if (rank > 0)
  {
    for (int partition_i = 0; partition_i < PARTITION_COUNT; partition_i++)
    {
      ...
      std::string stats_file_path = output_base_path + "/" + std::to_string(partition_i) + "/stats";
    ...
    for (int partition_i = 0; partition_i < PARTITION_COUNT; partition_i++)
    {
      ...
      {
        loadHashMap(local_hashmap, partition_i, file_i, output_base_path);
