...
int main(int argc, char *argv[])
{
  ...
  if (rank > 0)
  {
    ...
    Counter counter(kmer_size, READ_QUEUE_SIZE, &writer_queue, PARTITION_COUNT);
    ...
    // ===========================
      ...
      FILE *encoded_file = fopen(file_name , "rb"); //"encoded.bin"
      uint32_t current_header_block;
      size_t current_read_block_count;
      uint32_t current_read_length;
      uint32_t *encoded_read;
      char *decoded_read;
      const size_t bases_per_block = (size_t)10;
      const uint32_t bit_mask_for_character = (uint32_t)7;

      while (fread(&current_header_block, sizeof(uint32_t), 1, encoded_file))
      {
        assert((current_header_block >> 30) == ((uint32_t)3));
        current_read_length = current_header_block & ~(3 << 30);
        // cout << "current read length = " << current_read_length << std::endl;
        current_read_block_count = ((((size_t)(current_read_length)) + bases_per_block - 1) / bases_per_block);
        encoded_read = (uint32_t *)calloc(current_read_block_count, sizeof(uint32_t));
        // decoded_read = (char *)calloc(current_read_length + 1, sizeof(char));
        fread(encoded_read, sizeof(uint32_t), current_read_block_count, encoded_file);
        size_t read_character_i = 0;
        // for (size_t read_block_i = 0; read_block_i < current_read_block_count - 1; read_block_i++)
        // {
        //   for (size_t base_in_block_i = 0; base_in_block_i < bases_per_block; base_in_block_i++)
        //   {
        //     decoded_read[read_character_i++] = getDecodedCharacter((encoded_read[read_block_i] >> ((bases_per_block - base_in_block_i - 1) * 3)) & bit_mask_for_character);
        //   }
        // }

        // last block
        const size_t bases_in_last_block = current_read_length - (bases_per_block * (current_read_block_count - 1));
        // for (size_t base_in_block_i = 0; base_in_block_i < bases_in_last_block; base_in_block_i++)
        // {
        //   decoded_read[read_character_i++] = getDecodedCharacter((encoded_read[current_read_block_count - 1] >> ((bases_per_block - base_in_block_i - 1) * 3)) & bit_mask_for_character);
        // }
        // cout << decoded_read << std::endl;    // Uncomment this line to get the output in the terminal
        // free(decoded_read);

        // free(encoded_read);
        struct CounterArgumentsEnc *args = (struct CounterArgumentsEnc *)malloc(sizeof(struct CounterArgumentsEnc));
      
        args->buffer = encoded_read;
        args->bases_per_block = bases_per_block;
        args->current_read_length = current_read_length;
        args->current_read_block_count = current_read_block_count;
